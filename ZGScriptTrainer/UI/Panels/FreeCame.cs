...
{
    internal class FreeCamPanel : ZGPanel
    {
        public FreeCamPanel(UIBase owner) : base(owner)
        {
        }

        public override string Name => "自由摄像机";
        public override UIManager.Panels PanelType => UIManager.Panels.FreeCamWindow;
        ...
        {
            startStopButton = UIFactory.CreateButton(ContentRoot, "ToggleButton", "自由摄像机");
            ...
            GameObject toggleObj = UIFactory.CreateToggle(ContentRoot, "自由摄像机启动", out useGameCameraToggle, out Text toggleText);
            UIFactory.SetLayoutElement(toggleObj, minHeight: 25, flexibleWidth: 9999);
            useGameCameraToggle.onValueChanged.AddListener(OnUseGameCameraToggled);
            useGameCameraToggle.isOn = false;
            toggleText.text = "自由摄像机?";

            AddSpacer(5);

            GameObject posRow = AddInputField("Position", "自由摄像机 Pos:", "eg. 0 0 0", out positionInput, PositionInput_OnEndEdit);
            ...
            AddInputField("MoveSpeed", "移动速度:", "Default: 1", out moveSpeedInput, MoveSpeedInput_OnEndEdit);
            moveSpeedInput.Text = desiredMoveSpeed.ToString();

            AddSpacer(5);

            string instructions = @"按键设置:
- WASD : 移动
- Space : 上升
- LeftCtrl : 下降
- 鼠标右键: 自由视角
- Shift: 加速";
        ...
        void SetToggleButtonState()
        {
            if (inFreeCamMode)
            {
                ...
                startStopButton.ButtonText.text = "结束Freecam";
            }
            else
            {
                RuntimeHelper.SetColorBlockAuto(startStopButton.Component, new(0.2f, 0.4f, 0.2f));
                startStopButton.ButtonText.text = "启动Freecam";
            }
        }
